...
export async function extractReceiptData(file: File): Promise<ReceiptData> {
  ...
  try {
    ...
    if (!documents || documents.length === 0) {
      // If no documents found, try general document model
      const generalPoller = await client.beginAnalyzeDocument("prebuilt-document", buffer);
      const generalResult = await generalPoller.pollUntilDone();
      
      // Extract fields with fallback logic from general document
      const invoiceNumber = extractInvoiceNumberFromGeneral(generalResult);
      const date = extractDateFromGeneral(generalResult);
      const { productName, quantity, unit } = extractProductInfoFromGeneral(generalResult);
      
      // Calculate average confidence
      const confidence = calculateAverageConfidenceFromGeneral(generalResult);
...
function extractInvoiceNumberFromGeneral(result: any): string {
  // Try to find invoice number in key-value pairs
  if (result.keyValuePairs) {
    for (const pair of result.keyValuePairs) {
      const key = pair.key?.content?.toLowerCase() || "";
      if (key.includes("irsaliye") || key.includes("invoice") || key.includes("no")) {
        return pair.value?.content || "Bulunamadı";
      }
    }
  }
  
  // Fallback: Try to find with regex pattern in content
...
function extractDateFromGeneral(result: any): string {
  // Try to find date in key-value pairs
  if (result.keyValuePairs) {
    for (const pair of result.keyValuePairs) {
      const key = pair.key?.content?.toLowerCase() || "";
      if (key.includes("tarih") || key.includes("date")) {
        const content = pair.value?.content || "";
        // Try different date formats
        const datePatterns = [
          /(\d{2})[\/\-\.](\d{2})[\/\-\.](\d{4})/, // dd/mm/yyyy or dd-mm-yyyy or dd.mm.yyyy
          /(\d{4})[\/\-\.](\d{2})[\/\-\.](\d{2})/, // yyyy-mm-dd
  ...
  // Fallback: Try to find date with regex patterns in content
  const content = result.content || "";
...
function extractProductInfoFromGeneral(result: any): { productName: string; quantity: number; unit: string } {
  // Try to find product information in tables
  if (result.tables) {
    for (const table of result.tables) {
      // Look for rows that might contain product information
...
function calculateAverageConfidenceFromGeneral(result: any): number {
  // For general document, we'll return a default confidence
  return 0.7;
}